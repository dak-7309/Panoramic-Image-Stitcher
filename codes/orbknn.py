...
plt.savefig("orb_knn:_original_images_1_and_2.jpg")
plt.show()

#creating an orb object, followed by detecting and extracting relevant features from our given images
#  (keypointsA, featuresA) contains these these features
t_orb_knn_0 = time.time()

(keypointsA, featuresA) = cv2.ORB_create().detectAndCompute(img1_gray, None)
(keypointsB, featuresB) = cv2.ORB_create().detectAndCompute(img2_gray, None)
...
def plot_features(keypointsA,keypointsB,img1_gray,img2_gray):
    ...
    plt.savefig("orb_bf:features.jpg")
    plt.show()
...
plt.savefig("orb_knn:matches.jpg")
plt.show()
