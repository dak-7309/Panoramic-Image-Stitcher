...
plt.savefig("brisk_knn:_original_images_1_and_2.jpg")
plt.show()

#creating an brisk object, followed by detecting and extracting relevant features from our given images
#  (keypointsA, featuresA) contains these these features

t_brisk_knn = time.time()
(keypointsA, featuresA) = cv2.BRISK_create().detectAndCompute(img1_gray, None)
(keypointsB, featuresB) = cv2.BRISK_create().detectAndCompute(img2_gray, None)
...
def plot_features(keypointsA,keypointsB,img1_gray,img2_gray):
    ...
    plt.savefig("brisk_bf:features.jpg")
    plt.show()
...
plt.imshow(img3)
plt.savefig("brisk_bf:matches.jpg")
plt.show()
...
def transform_threshold(finale):
    ...
    # convert the processed RGB to grayscale
    ...
    plt.imshow(final)
    plt.savefig("brisk_bf:final_transformed_image.jpg")



# calling the transform function to the final 
transform_threshold(finale)
t_brisk_bf_1 = time.time()
